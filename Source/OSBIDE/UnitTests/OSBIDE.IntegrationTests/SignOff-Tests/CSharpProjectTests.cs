...
namespace OSBIDE.IntegrationTests.IntegrationTests
{
    [TestClass]
    public class CSharpProjectTests
    {
        ...
        public void WinformsApplication()
        {
            UIThreadInvoker.Invoke((ThreadInvoker)delegate()
            {
                ...
                testUtils.CreateProjectFromTemplate("MyWindowsApp", "WPF Application", "CSharp", false);
                Assert.AreEqual<int>(1, testUtils.ProjectCount());

                //force save
                testUtils.ForceSaveSolution();

                //trigger debug (and hopefully build as well)
                testUtils.ExecuteCommand("Debug.Start");
