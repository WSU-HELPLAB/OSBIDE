...
namespace OSBIDE.Web.Controllers
{
    ...
    public class GradebookController : ControllerBase
    {
        public GradebookController()
            : base()
        {
            ViewBag.Courses = Db.CourseUserRelationships
                .Where(cur => cur.UserId == CurrentUser.Id)
                .Where(cur => cur.Course.IsDeleted == false)
                .Select(cur => cur.Course)
                .ToList();
        }
        ...
        /// <returns></returns>
        private List<List<string>> ParseStudentTable(CourseUserRelationship courseUser, List<List<string>> gradebookTable)
        {
            ...
            if (gradebookTable.Count > 0)
            {
                ...
                for (int i = 0; i < gradebookTable.Count; i++)
                {
                    ...
                    int studentId = -1;
                    Int32.TryParse(gradebookTable[i][IdColumn], out studentId);
                    if (studentId == courseUser.User.InstitutionId)
                    {
